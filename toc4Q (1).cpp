...
int belongsToLanguage(char* str, int start, int end)
{
    ...
    if (str[start] == '1' && str[end] == '1' && belongsToLanguage(str, start + 1, end - 1)) {
        return 1;
    }
    if (str[start] == '0' && belongsToLanguage(str, start + 1, end)) {
        return 1;
    }
    if (str[start] == '1' && belongsToLanguage(str, start + 1, end)) {
        return 1;
    }
    if (str[end] == '0' && belongsToLanguage(str, start, end - 1)) {
        return 1;
    }
    if (str[end] == '1' && belongsToLanguage(str, start, end - 1)) {
        return 1;
    }
    return 0;
}
